...
impl Bme280 {
    ...
    pub fn make_measurement(&self) -> Result<Measurement, Error> {
        ...
        let pres_raw: i32 = ((data[0] as i32) << 12) | ((data[1] as i32) << 4) | ((data[2] as i32) >> 4);
        let temp_raw: i32 = ((data[3] as i32) << 12) | ((data[4] as i32) << 4) | ((data[5] as i32) >> 4);
...
fn refine_temperature(temp_raw: i32, calibration: &CalibrationData) -> TemperatureData {
    let var1: f64 = ((temp_raw as f64) / 16384.0 - (calibration.dig_t1 as f64) / 1024.0) * (calibration.dig_t2 as f64);
    let var2: f64 = (((temp_raw as f64) / 131072.0 - (calibration.dig_t1 as f64) / 8192.0) *
        ((temp_raw as f64) / 131072.0 - (calibration.dig_t1 as f64) / 8192.0)) * (calibration.dig_t3 as f64);
...
fn refine_pressure(pres_raw: i32, calibration: &CalibrationData, t_fine: i32) -> f64 {
    ...
    var1 = ((calibration.dig_p3 as f64) * var1 * var1 / 524288.0 + (calibration.dig_p2 as f64) * var1) / 524288.0;
...
fn refine_humidity(hum_raw: i32, calibration: &CalibrationData, t_fine: i32) -> f64 {
    ...
    var_h = ((hum_raw as f64) - ((calibration.dig_h4 as f64) * 64.0 + (calibration.dig_h5 as f64) / 16384.0 *
        var_h)) * ((calibration.dig_h2 as f64) / 65536.0 * (1.0 + (calibration.dig_h6 as f64) / 67108864.0 * var_h *
